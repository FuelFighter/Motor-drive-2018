...
static uint8_t PWM_Counter = 0; //Counter for shutting of drivers in PWM mode
//Make a define for counter in motor_controller_selection or similar? 
...
void controller(volatile ModuleValues_t *vals){
	...
	}else if (vals->ctrl_type == PWM)
	{
		...
		if (PWM_Counter < 100)
		{
			//100 = 400ms. Must be long enough so that the motor has time to sync,
			//but short enough so that the actuator does not reach the wheel.
			//This code is probably better to run in the actuator board.
			drivers(0); //Shut off driver to avoid applying torque when engaging.
			PWM_Counter = 0; //Reset PWM_Counter
		}
